...
class AnyLanguageExpressaicGUI:
    ...
    def build_body(self):
        ...
        self.build_main_panel(body)

    def build_sidebar(self, body):
        sidebar = ttk.Frame(body, style="Sidebar.TFrame", width=300)
        ...
        self.file_tree = ttk.Treeview(sidebar, show="tree")
    ...
    def build_language_bar(self, parent):
        ...
        self.language_dropdown = ttk.Combobox(bar, state="readonly", width=18)
        ...
        self.selected_label.pack(side=tk.LEFT, padx=12)
    ...
    def build_table_view(self):
        ...
        scroll_y = ttk.Scrollbar(container, orient="vertical", command=self.table.yview)
        scroll_y.pack(side=tk.RIGHT, fill=tk.Y)

        scroll_x = ttk.Scrollbar(self.table_frame, orient="horizontal", command=self.table.xview)
    ...
    def populate_tree(self):
        ...
        root_id = self.file_tree.insert("", tk.END, text=self.base_folder.name, open=True, values=[str(self.base_folder)])

        self.add_tree_nodes(root_id, self.base_folder)

    def add_tree_nodes(self, parent_id, folder_path):
        try:
        ...
        for entry in entries:
            if entry.is_dir():
                child_id = self.file_tree.insert(parent_id, tk.END, text=f"📁 {entry.name}", open=False, values=[str(entry)])
                self.add_tree_nodes(child_id, entry)
            elif entry.suffix.lower() in [".yaml", ".yml"]:
                self.file_tree.insert(parent_id, tk.END, text=f"📄 {entry.name}", values=[str(entry)])
    ...
    def show_table_view(self):
        ...
        for col in columns:
            self.table.heading(col, text=clean_title(col))
            self.table.column(col, width=290, anchor="w")
